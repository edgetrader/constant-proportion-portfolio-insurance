...
def get_hfi_returns():
    ...
    """
    hfi = pd.read_csv(DATA_FOLDER + "edhec-hedgefundindices.csv",
...
def plot_ef(n_points, er, cov, style='.-', legend=False, show_cml=False, riskfree_rate=0, 
                show_ew=False, show_gmv=False, show_erc=False):
    """
    ...
    if show_erc:
        ...
        # add GMV
        ax.plot([vol_erc], [r_erc], color='red', marker='o', markersize=10)
...
def funding_ratio_2(assets, liabilities, r):
    """
    Computes the funding ratio of a series of liabilities, based on an interest rate and current value of assets
    """
    return pv_2(assets, r)/pv_2(liabilities, r)
...
def bond_price_2(maturity, principal=100, coupon_rate=0.03, coupons_per_year=12, discount_rate=0.03):
    """
    ...
    if isinstance(discount_rate, pd.DataFrame):
        ...
        return pv_2(cash_flows, discount_rate/coupons_per_year)
...
def macaulay_duration_2(flows, discount_rate):
    """
    Computes the Macaulay Duration of a sequence of cash flows, given a per-period discount rate
    """
    discounted_flows = discount_2(flows.index, discount_rate)*flows
...
def weight_msr(r, cov_estimator=sample_cov, **kwargs):
    ...
    """
    rfr = riskfree_rate(r, **kwargs)
    er = annualize_rets(r, **kwargs)
    est_cov = cov_estimator(r, **kwargs)
    return msr(riskfree_rate=rfr, er=er, cov=est_cov)

def riskfree_rate(r, rfr_series, **kwargs):
    """
    Extract the latest risk free rate based on the return series r
    """
    return rfr_series.loc[r.index][-1]
...
def backtest_ws(r, estimation_window=60, weighting=weight_ew, verbose=False, **kwargs):
    ...
    weighting: the weighting scheme to use, must be a function that takes "r", and 
    a variable number of keyword-value arguments
    """
    n_periods = r.shape[0]
    # return windows
